...
void
AgentInterface::InitializeAgent()
{
  //	TODO
  if(removed)
	return;
...
AgentInterface::processEvent(const EventQueue::eEvent *event)
{
  if(removed)
	return NULL;
	  ...
	  if(event->propagationSpeed == 0)
	  ...
	  else
		{
		  ievent->activationTime =
			  Phys::speedOfEvent(event->posX, event->posY, posX, posY, event->propagationSpeed) + 1;
		}
...
AgentInterface::takeStep()
{
  ...
  try
	...
	{
	  Output::Inst()
		  ->kprintf("<b><font color=\"red\">Exception on takeStep. %s, %s</font></b></>", filename.c_str(), e
			  .what());
 ...
 */
std::unique_ptr<EventQueue::eEvent>
AgentInterface::handleEvent(std::unique_ptr<EventQueue::iEvent> eventPtr)
{
  if(removed)
	return NULL;
  if(nofile)
  ...
  try
	...
	{
	  Output::Inst()
		  ->kprintf("<b><font color=\"red\">Exception on event handling.%s, %s</font></b></>", filename.c_str(), e
			  .what());
...
//	Interface
void
AgentInterface::debug(std::string string)
{
  string.resize(4096);
...
void
AgentInterface::print(std::string string)
{
  string.resize(4096);
  Output::Inst()->kdebug(string.c_str());
}
...
unsigned long long
AgentInterface::generateEventID()
{
...
//	Physics
unsigned long long
AgentInterface::speedOfSound(double x_origin, double y_origin, double x_dest, double y_dest, double propagationSpeed)
{
  return Phys::speedOfEvent(x_origin, y_origin, x_dest, y_dest, propagationSpeed);
}

double
AgentInterface::distance(double x_origin, double y_origin, double x_dest, double y_dest)
{
  return Phys::calcDistance(x_origin, y_origin, x_dest, y_dest);
}

unsigned long long
AgentInterface::currentTime()
{
  return Phys::getCTime();
}

double
AgentInterface::currentTimeS()
{
  return (double) Phys::getCTime() * Phys::getTimeRes();
}

int
AgentInterface::getMacroFactor()
{
  return Phys::getMacroFactor();
}

double
AgentInterface::getTimeResolution()
{
  return Phys::getTimeRes();
}

double
AgentInterface::getMersenneFloat(double low, double high)
{
  return Phys::getMersenneFloat(low, high);
}

int64_t
AgentInterface::getMersenneInteger(int64_t low, int64_t high)
{
  int64_t number = low;
...
//	Map and position
std::vector<int>
AgentInterface::getEnvironmentSize()
{
  std::vector<int> tmp;
...
bool
AgentInterface::modifyMap(double x, double y, int r, int g, int b)
{
...
rgba
AgentInterface::checkMap(double x, double y)
{
...
bool
AgentInterface::checkMapAndChange(double x, double y, int r1, int g1, int b1, int r2, int b2, int g2)
{
  int modX = x * Phys::getScale();
  int modY = y * Phys::getScale();
...
int
AgentInterface::radialMapColorScan(int radius, int x, int y, int r, int g, int b)
{
  //  TODO
  int posX = x - radius;
  int posY = y - radius;
...
int
AgentInterface::radialMapScan(int radius, int x, int y)
{
  //  TODO
  int posX = x - radius;
  int posY = y - radius;
...
void
AgentInterface::addPosition(int x, int y, int id)
{
  GridMovement::addPos(x, y, id);
}

std::list<int>
AgentInterface::checkPosition(int x, int y)
{
...
void
AgentInterface::updatePosition(int oldX, int oldY, int newX, int newY, int id)
{
...
bool
AgentInterface::checkCollision(int x, int y)
{
  return GridMovement::checkCollision(posX, posY);
}

bool
AgentInterface::checkCollisionRadial(int radius, int x, int y)
{
  int posX = x - radius;
  int posY = y - radius;
...
int
AgentInterface::getMaskRadial(int radius, int x, int y)
{
  //  TODO
  int posX = x - radius;
  int posY = y - radius;
...
void
AgentInterface::gridMove(int oldX, int oldY, int newX, int newY)
{

}

double
AgentInterface::getGridScale()
{
  return GridMovement::getScale();
}

void
AgentInterface::initializeGrid()
{
  GridMovement::initGrid(1);
}

int
AgentInterface::radialCollisionScan(int radius, int x, int y, int id)
{
  //  TODO
  int posX = x - radius;
  int posY = y - radius;
...
bool
AgentInterface::updatePositionIfFree(int x1, int y1, int x2, int y2, int id)
{
...
//	Shared values
std::string
AgentInterface::getSharedNumber(std::string key)
{
...
void
AgentInterface::addSharedNumber(std::string key, double value)
{
  Shared::addNumber(key, value);
}

std::string
AgentInterface::getSharedString(std::string key)
{
  return Shared::getString(key);
}

void
AgentInterface::addSharedString(std::string key, double value)
{
...
//	Simulation core
void
AgentInterface::stopSimulation()
{
  Output::RunSimulation.store(false);
}

std::vector<std::string>
AgentInterface::getAgentPath()
{
...
int
AgentInterface::addAgent(int posX, int posY, int posZ, std::string path, std::string filename)
{
  return Interfacer::addLuaAgent(posX, posY, posZ, path, filename);
}

bool
AgentInterface::removeAgent(int id)
{
...
//	Agents
void
AgentInterface::emitEvent(int id, int posX, int posY, double speed, std::string tableString, std::string desc, int targetID, int targetGroup)
{
  //  TODO
...
bool
AgentInterface::addGroup(int groupID, int id)
{
...
bool
AgentInterface::removeGroup(int groupID, int id)
{
  auto autonPtr = Interfacer::getAgentPtr(id);
  if(autonPtr != NULL)
	{
...
void
AgentInterface::setMacroFactorMultipler(int id, int macroFactorMultiple)
{
  auto autonPtr = Interfacer::getAgentPtr(id);
...
bool
AgentInterface::changeAgentColor(int id, int r, int g, int b, int alpha)
{
...
//	Panic
void
AgentInterface::panic(std::string string)
{
...
//  Post processing
void
AgentInterface::processFunction(EventQueue::dataEvent *devent, double mapRes, double x, double y, double &zvalue, double &duration)
{
  //  TODO
  if(removed)
	return;
...
AgentInterface::getSyncData()
{
  ...
  try
	...
	{
	  Output::Inst()
		  ->kprintf("<b><font color=\"red\">Exception on retrieving sync data, %s</font></b></>", e.what());
	  Output::RunSimulation.store(false);
	  return;
	}
}
		  ...
		  if(int(posX * GridMovement::getScale()) != int(newPosX * GridMovement::getScale()) ||
			 int(posY * GridMovement::getScale()) != int(newPosY * GridMovement::getScale()))
			{
			  GridMovement::updatePos(
				  int(posX * GridMovement::getScale()), int(posY * GridMovement::getScale()), int(
					  newPosX * GridMovement::getScale()), int(newPosY * GridMovement::getScale()), ID);
			}
		}
