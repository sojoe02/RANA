 ...
 */
void
EventQueue::insertEEvent(std::unique_ptr<eEvent> eeventPtr)
{
  //std::lock_guard<std::mutex> lk(eEventMutex);
  eSize++;

  unsigned long long tmu = eeventPtr.get()->activationTime;
 ...
 */
std::list<std::unique_ptr<EventQueue::eEvent>>
EventQueue::getEEventList(unsigned long long tmu)
{
 ...
 */
bool
EventQueue::eEventsAtTime(unsigned long long tmu)
{
...
EventQueue::addUsedEEvent(std::unique_ptr<eEvent> eEvent)
{
  int id = eEvent.get()->id;
...
void
EventQueue::decrementEeventCounter(unsigned long long id)
{
  ...
  if(itr != usedEEvents.end())
	...
	Outbound::Inst()->simOutput("event not found in Used Eevent hash map, something is wrong");
}

void
EventQueue::incrementEeventCounter(unsigned long long id)
{
  auto itr = usedEEvents.find(id);

  if(itr != usedEEvents.end())
	{
	  itr->second.get()->reference_count++;
	}
  else
	Outbound::Inst()->simOutput("external event not found in Event hash map, cannot increment reference counter");
}
 ...
 */
void
EventQueue::insertIEvent(std::unique_ptr<iEvent> ieventPtr)
{
  //std::lock_guard<std::mutex> lk(iEventMutex);
  //put event in hashmap.
  iSize++;
  unsigned long long tmu;
  tmu = ieventPtr->activationTime;
 ...
 * @returns event list at current tmu index in the hashmap, or NULL if there isn't an eventlist
 */
std::list<std::unique_ptr<EventQueue::iEvent>>
EventQueue::getIEventList(unsigned long long tmu)
{
 ...
 */
bool
EventQueue::iEventsAtTime(unsigned long long tmu)
{
 ...
 */
void
EventQueue::saveEEventData(std::string path, std::string luaFileName, int autonAmount, double areaY, double areaX)
{
...
//  Outbound::Inst()->simOutput("Saving event data to file:  %s\n", path.c_str());
...
//  posBufferPath.append("/positionData/_positionData.pos");
...
EventQueue::printATmus()
{
  ...
  for(activeIt = activeTmu.begin(); activeIt != activeTmu.end(); activeIt++)
	{
      Outbound::Inst()->simOutput("%llu\n", *activeIt);
	}
  Outbound::Inst()->simOutput("----------------\n");
}

void
EventQueue::addAgentInfo(int id, std::string filename)
{
